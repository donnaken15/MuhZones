	...
	public class Zones : IDisposable
	{
		...
		public class RawImg : IDisposable
		{
				...
				get
				{
					...
					else
					{
						...
						/*if (Image.RawFormat == ImageFormat.Png)
							...
							return "bmp";
						else if (Image.RawFormat == ImageFormat.Tiff)
							return "tif";
						else
							return ".";
					...
					/*if (value != null)
					{
						switch (Type.GetTypeCode(value.GetType()))
						{
							...
							case TypeCode.UInt32:
								head.key = Convert.ToUInt32(value);
								return;
							case TypeCode.String:
								head.key = QbKey.Create(value.ToString()).Crc;
								return;
						}
					}*/
					head.key = value.Crc;
			...
			public RawImg(byte[] img) // lazy copy again lol
			{
				...
				{
					Console.WriteLine("fail "+Eswap(Bit.ToUInt32(img,0)).ToString("X8"));
		...
		public class GFX
		{
			private const int maxCount = 128; // for now
			private struct Head
			{
				public uint magic;
				public ushort unk1;
				public ushort count; // ugh what do i do here
				public uint start;
				// caps at 128 textures,
				// otherwise crash because
				// apparently image data given
				// to D3D is a texture vtable
		...
		public class Scene
		{
			...
			public abstract/* new to this, and i already hate it */ class MatBase
			{
				...
				/*public Vec4[] props_base(int c, int a)
				{
					if (c == 0) return null;
					Vec4[] v = new Vec4[c];
					for (int i = 0; i < c; i++)
				...
				/*public Vec4[] Props
				{
					get { return props_base(propCount, propOffset); }
				}
				public Vec4[] Props2
				{
					get { return props_base(prop2Count, prop2Offset); }
				}*/
		...
		{
			string pbo = fname.Replace(".pab",".pak");
			string pko = fname.Replace(".pak",".pab");
			File.Copy(pf.PakFilename, pbo, true);
			File.Copy(pf.PabFilename, pko, true);
			pf = new PakFormat(pko, pbo, "", PakFormatType.PC);
			pe = new PakEditor(pf, false);
			//Save();
		}*/
	}
